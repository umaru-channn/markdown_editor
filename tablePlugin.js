 ...
 * This prevents index misalignment when external changes (like auto-inserted newlines) shift the table position.
...
const Cell = ({
    ...
    onResizeStart, isSelected, isEditing, caretPosition,
    ...
    const resizeHandle = isHeader ? html`
        <div 
            class="cm-table-resize-handle" 
...
const TableComponent = ({ initialData, initialWidths, onUpdate, onRender }) => {
    ...
    // Context Menu State
    const [contextMenu, setContextMenu] = useState(null); // { x, y, rowIndex, colIndex }

    // Delete Double-Click Detection
    ...
    // --- Insert/Delete Logic for Context Menu ---
    const insertRow = (index) => {
        const newData = { ...data };
        const newRow = new Array(newData.headers.length).fill("");

        // index: -1=before header(unlikely), 0=first row, ...
    ...
    const insertColumn = (index) => {
        ...
        const newWidths = [...widths];
        newWidths.splice(spliceIndex, 0, 100); // default width
        setWidths(newWidths);
    ...
    const handleMenuAction = (action, rowIndex, colIndex) => {
        ...
        switch (action) {
            case 'insertRowAbove':
                insertRow(rowIndex === -1 ? 0 : rowIndex);
                break;
            case 'insertRowBelow':
    ...
    // --- Deletion Logic (Selection based) ---
    ...
    return html`
        ...
        const isDragTarget = dragState.type === 'col' && dragState.toIndex === i;
        let dragClass = '';
        if (isDragTarget && dragState.fromIndex !== i) {
        ...
        return html`
                ...
                <tbody>
                    ${data.rows.map((row, rI) => html`
                        <tr key=${rI}>
                            ${row.map((cell, cI) => {
        const isDragTarget = dragState.type === 'row' && dragState.toIndex === rI;
        let dragClass = '';
        if (isDragTarget && dragState.fromIndex !== rI) {
        ...
        return html`
            ...
            <button 
                class="cm-table-add-col-btn" 
                onClick=${addColumnBtn}
                title="列を追加"
            >
                +
            </button>
        ...
        enter: (node) => {
            if (node.name === "Table") {
                ...
                if (startLine.number > 1) {
                    ...
                    if (prevLine.text.match(/^<!--\s*table-widths:/)) {
                        effectiveStartLine = prevLine;
                    }
                }
                ...
                if (effectiveStartLine.number > 1) {
                    ...
                    if (lineAbove.length > 0) {
                        changes.push({ from: effectiveStartLine.from, insert: "\n" });
                    }
                }
                ...
                if (tableTo > 0) {
                    ...
                    if (lastTableLine.number < state.doc.lines) {
                        ...
                        if (lineBelow.length > 0) {
                            changes.push({ from: lineBelow.from, insert: "\n" });
                        }
                    }
                }
            }
...
const tableStyles = EditorView.baseTheme({
    ...
    ".cm-table-widget-container:focus": {
        outline: "none"
    },
    /* Table Layout Wrapper - CSS Grid */
    ".cm-table-widget-wrapper": {
        display: "grid",
        gridTemplateColumns: "max-content 24px", /* Table Width | Col Button Width */
        gridTemplateRows: "max-content 24px",    /* Table Height | Row Button Height */
        width: "fit-content",
    ...
    ".cm-table-drag-handle:hover": {
        backgroundColor: "rgba(0, 0, 0, 0.05)",
        color: "#666"
    },
    ".cm-table-cell:hover .cm-table-drag-handle": {
        opacity: 1
    },
    /* Column Drag Handle (Top) */
    ".col-drag-handle": {
        top: "-8px",
        left: "50%",
    ...
    /* Drag Targets Highlight (Fixed Logic) */
    ".drag-target-left": {
        borderLeft: "2px solid #2383e2 !important",
        backgroundColor: "rgba(35, 131, 226, 0.1)"
    },
    ".drag-target-right": {
        borderRight: "2px solid #2383e2 !important",
        backgroundColor: "rgba(35, 131, 226, 0.1)"
    },
    ".drag-target-top": {
        borderTop: "2px solid #2383e2 !important",
        backgroundColor: "rgba(35, 131, 226, 0.1)"
    },
    ".drag-target-bottom": {
        borderBottom: "2px solid #2383e2 !important",
        backgroundColor: "rgba(35, 131, 226, 0.1)"
    },
    /* Content */
    ...
    ".cm-table-cell-content.view-mode": {
        pointerEvents: "none"
    },
    ...
    ".cm-table-context-menu .menu-item": {
        padding: "6px 12px",
        cursor: "pointer"
    },
    ".cm-table-context-menu .menu-item:hover": {
        backgroundColor: "#f0f0f0"
    },
    ".cm-table-context-menu .menu-separator": {
        height: "1px",
        backgroundColor: "#eee",
        margin: "4px 0"
    },
    /* Dark Mode Adjustments */
    "&dark .cm-table-add-col-btn": {
        backgroundColor: "#161b22",
        borderColor: "#30363d",
        color: "#8b949e"
    },
    "&dark .cm-table-add-col-btn:hover": {
        backgroundColor: "#1f2428",
        color: "#c9d1d9"
    },
    "&dark .cm-table-add-row-btn": {
        backgroundColor: "#161b22",
        borderColor: "#30363d",
        color: "#8b949e"
    },
    "&dark .cm-table-add-row-btn:hover": {
        backgroundColor: "#1f2428",
        color: "#c9d1d9"
    },
    ...
    "&dark .cm-table-context-menu": {
        backgroundColor: "#2d2d2d",
        border: "1px solid #444",
        color: "#ccc"
    },
    "&dark .cm-table-context-menu .menu-item:hover": {
        backgroundColor: "#3e3e3e"
    },
    "&dark .cm-table-context-menu .menu-separator": {
        backgroundColor: "#444"
    }
});
